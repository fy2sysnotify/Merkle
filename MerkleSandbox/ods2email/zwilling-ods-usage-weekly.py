...
# Set script name
script_name = 'zwilling-ods-usage-weekly'

# Set credits start date - must be in ISO 8601 format
credits_start_date = '2021-01-01'

last_week = datetime.datetime.now() + dateutil.relativedelta.relativedelta(weeks=-1)
usage_start_date = last_week.strftime('%Y-%m-%d')
...
def config_connection(email, email_sender, recipient, email_cc):
    ...
    server.sendmail(email_sender, [recipient, email_cc], text)
...
worksheet_header = ['SandboxID', 'State', 'Created at', 'Minutes up in last 7 days',
                    'Minutes down in last 7 days', 'Credits since 01.01.2021']
...
for i in sandbox_list['data']:
    ...
    if realm == 'bcgv':  # Smenyash ID-to mezhdu kavichkite s tova na klienta, koito iskash da reportvash. ID-to se vzima ot https://wiki.isobarsystems.com/display/SUPTEAM/Local+POD+time+-+Notifications
        ...
        credits_spent = minutes_up + (minutes_down * 0.3)
        # list2fill.append(credits_spent)
        logger.debug(f'List to fill with parsed values from second request endpoint is {list2fill}')

        # Make request with sandboxIDs to get each Sandbox usage for the last 7 days
        log_and_print('Third request endpoint - Sandbox usage for the last 7 days')

        sandbox_usage_last_7_days = None

        try:
            sandbox_usage_last_7_days = requests.get(
            ...
            email_recipient = const.my_business_email
            email_subject = f'Failed to get Sandbox usage for the last 7 days in cron job - {script_name}'
            email_text = f'Failed to get Sandbox usage for the last 7 days in cron job - {script_name}. Check the attached logs'
            email_attachment = str(Path.cwd() / script_log_file)
            send_email(email_recipient, email_subject, email_text, email_attachment)
            raise

        logger.debug(f'Third request endpoint answer is {sandbox_usage_last_7_days}')

        minutes_up_last_7_days = sandbox_usage_last_7_days['data']['minutesUp']
        list2fill.append(minutes_up_last_7_days)
        minutes_down_last_7_days = sandbox_usage_last_7_days['data']['minutesDown']
        list2fill.append(minutes_down_last_7_days)
        ...
        log_and_print(f'Fourth request endpoint - get Sandbox usage since {credits_start_date}')

        sandbox_usage_since2021_01_01 = None

        try:
            sandbox_usage_since2021_01_01 = requests.get(
        ...
        logger.debug(f'Fourth request endpoint answer is {sandbox_usage_since2021_01_01}')

        minutes_up210101 = sandbox_usage_since2021_01_01['data']['minutesUp']
        minutes_down210101 = sandbox_usage_since2021_01_01['data']['minutesDown']
        credits_spent = minutes_up210101 + (minutes_down210101 * 0.3)
        list2fill.append(credits_spent)
...
log_and_print(f'Fifth request endpoint - get Sandbox usage on realm bcgv since {credits_start_date}')

sandbox_usage_on_realm_bcgv_since2021_01_01 = None

try:
    sandbox_usage_on_realm_bcgv_since2021_01_01 = requests.get(
        f"https://admin.us01.dx.commercecloud.salesforce.com/api/v1/realms/bcgv/usage?from={credits_start_date}",
        headers=headers).json()
except Exception as conn_error:
    logger.error(conn_error)
    logger.debug('Failed to get Sandbox usage on realm bcgv from fifth request endpoint')
    email_recipient = const.my_business_email
    email_subject = f'Failed to get Sandbox usage on realm bcgv since {credits_start_date} in cron job - {script_name}'
    email_text = f'Failed to get Sandbox usage on realm bcgv since {credits_start_date} in cron job - {script_name}. Check the attached logs'
    email_attachment = str(Path.cwd() / script_log_file)
    send_email(email_recipient, email_subject, email_text, email_attachment)
    raise

logger.debug(f'Fifth request endpoint answer is {sandbox_usage_on_realm_bcgv_since2021_01_01}')
...
log_and_print(credits_spent_on_realm_bcgv)
...
last_dataframe_row = {'SandboxID': '', 'State': '', 'Created at': '',
                      'Minutes up in last 7 days': '', 'Minutes down in last 7 days': '',
                      'Credits since 01.01.2021': credits_spent_on_realm_bcgv}
...
emailText = f'Dear Zwilling, my name is {script_name}. Please check the overview for the last 7 days. Keep in mind that living sandboxes on demand are spending credits. Each minute up time for Medium ODS = 1 credit. And each minute down time = 0.3 credit. If you have any questions, please do not hesitate to contact Merkle Operations Support.'

email_recipient = const.my_business_email
email_subject = 'Zwilling ODS usage for the last 7 days'
