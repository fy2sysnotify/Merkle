...
# Set script name
script_name = 'shiseido-emea-ods-usage-weekly'
...
worksheet_header = [
    'SandboxID', 'State', 'Created at', 'Minutes up in last 7 days',
    'Minutes down in last 7 days', 'Credits since 18.03.2022'
]
...
for i in sandbox_list['data']:
    ...
    if realm == 'bcmq':  # Smenyash ID-to mezhdu kavichkite s tova na klienta, koito iskash da reportvash. ID-to se vzima ot https://wiki.isobarsystems.com/display/SUPTEAM/Local+POD+time+-+Notifications
        ...
        # Make request with sandboxIDs to get Sandbox usage since 2022-03-18
        log_and_print(f'Fourth request endpoint - get Sandbox usage since {credits_start_date}')

        sandbox_usage_since2022_03_18 = None

        try:
            sandbox_usage_since2022_03_18 = requests.get(
        ...
        logger.debug(f'Fourth request endpoint answer is {sandbox_usage_since2022_03_18}')

        minutes_up220318 = sandbox_usage_since2022_03_18['data']['minutesUp']
        minutes_down220318 = sandbox_usage_since2022_03_18['data']['minutesDown']
        credits_spent = minutes_up220318 + (minutes_down220318 * 0.3)
        list2fill.append(credits_spent)
...
# Get usage for realm bcmq since 18.03.2022
log_and_print(f'Fifth request endpoint - get Sandbox usage on realm {realm} since {credits_start_date}')
...
try:
    ...
    logger.error(conn_error)
    logger.debug(f'Failed to get Sandbox usage on realm {realm} from fifth request endpoint')
    email_recipient = const.my_business_email
    email_subject = f'Failed to get Sandbox usage on realm {realm} since {credits_start_date} in cron job - {script_name}'
    email_text = f'Failed to get Sandbox usage on realm {realm} since {credits_start_date} in cron job - {script_name}. Check the attached logs'
    email_attachment = str(Path.cwd() / script_log_file)
    send_email(email_recipient, email_subject, email_text, email_attachment)
    raise
