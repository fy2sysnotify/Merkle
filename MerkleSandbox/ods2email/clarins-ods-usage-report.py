...
# Set script name
script_name = 'clarins-ods-usage-report'
...
logging.basicConfig(filename=f'{script_name}_{const.today_is}.log',
                    level=logging.DEBUG,
                    format=log_format)
...
for i in sandbox_list['data']:
    realm = i['realm']
    realm_name = ''
    if realm == 'aafs':  # Smenyash ID-to mezhdu kavichkite s tova na klienta, koito iskash da reportvash. ID-to se vzima ot https://wiki.isobarsystems.com/display/SUPTEAM/Local+POD+time+-+Notifications
        realm_name = 'Clarins'  # Smenyash imeto mezhdu kavichkite s tova na klienta, koito iskash da reportvash.
        sandbox_id = i['id']
...
log_and_print(f'Fifth request endpoint - get Sandbox usage on realm aafs since {credits_start_date}')

sandbox_usage_on_realm_aafs_since2021_01_01 = None

try:
    sandbox_usage_on_realm_aafs_since2021_01_01 = requests.get(
        f"https://admin.us01.dx.commercecloud.salesforce.com/api/v1/realms/aafs/usage?from={credits_start_date}",
        headers=headers).json()
except Exception as conn_error:
    logger.error(conn_error)
    logger.debug('Failed to get Sandbox usage on realm aafs from fifth request endpoint')
    email_recipient = const.my_business_email
    email_subject = f'Failed to get Sandbox usage on realm aafs since {credits_start_date} in cron job - {script_name}_sfcc'
    email_text = f'Failed to get Sandbox usage on realm aafs since {credits_start_date} in cron job - {script_name}_sfcc. Check the attached logs'
    email_attachment = str(Path.cwd() / script_log_file)
    send_email(email_recipient, email_subject, email_text, email_attachment)
    raise

logger.debug(f'Fifth request endpoint answer is {sandbox_usage_on_realm_aafs_since2021_01_01}')

realm_aafs_minutes_up210101 = sandbox_usage_on_realm_aafs_since2021_01_01['data']['minutesUp']
realm_aafs_minutes_down210101 = sandbox_usage_on_realm_aafs_since2021_01_01['data']['minutesDown']
credits_spent_on_realm_aafs = realm_aafs_minutes_up210101 + (realm_aafs_minutes_down210101 * 0.3)
log_and_print(credits_spent_on_realm_aafs)
...
last_dataframe_row = {'SandboxID': '', 'Realm': '', 'Instance': '', 'State': '', 'Created at': '', 'Created by': '',
                      ...
                      'Credits since 01.01.2021': credits_spent_on_realm_aafs}
...
email_recipient = 'borislav.doychev@merklecxm.com,aleksandar.chomakov@merklecxm.com,dessislava.popova@merkle.com'
email_subject = 'Clarins ODS update for today'
