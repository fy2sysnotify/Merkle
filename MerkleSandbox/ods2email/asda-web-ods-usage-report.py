...
# Set script name
script_name = 'asda-web-ods-usage-report'
...
for i in sandbox_list['data']:
    ...
    if realm == 'aaay':  # Smenyash ID-to mezhdu kavichkite s tova na klienta, koito iskash da reportvash. ID-to se vzima ot https://wiki.isobarsystems.com/display/SUPTEAM/Local+POD+time+-+Notifications
        realm_name = 'web'  # Smenyash imeto mezhdu kavichkite s tova na klienta, koito iskash da reportvash.
        sandbox_id = i['id']
        ...
        credits_spent_on_realm_aaay = minutes_up + (minutes_down * 0.3)
        list2fill.append(credits_spent_on_realm_aaay)
        ...
        sandbox_usage_on_realm_aaay_since2021_03_01 = None

        try:
            sandbox_usage_on_realm_aaay_since2021_03_01 = requests.get(
        ...
        logger.debug(f'Fourth request endpoint answer is {sandbox_usage_on_realm_aaay_since2021_03_01}')

        realm_aaay_minutes_up210301 = sandbox_usage_on_realm_aaay_since2021_03_01['data']['minutesUp']
        realm_aaay_minutes_down210301 = sandbox_usage_on_realm_aaay_since2021_03_01['data']['minutesDown']
        credits_spent_on_realm_aaay = realm_aaay_minutes_up210301 + (realm_aaay_minutes_down210301 * 0.3)
        list2fill.append(credits_spent_on_realm_aaay)
...
log_and_print(f'Fifth request endpoint - get Sandbox usage on realm aaay since {credits_start_date}')

sandbox_usage_on_realm_aaay_since2021_03_01 = None

try:
    sandbox_usage_on_realm_aaay_since2021_03_01 = requests.get(
        f"https://admin.us01.dx.commercecloud.salesforce.com/api/v1/realms/aaay/usage?from={credits_start_date}",
        headers=headers).json()
except Exception as conn_error:
    logger.error(conn_error)
    logger.debug('Failed to get Sandbox usage on realm aaay from fifth request endpoint')
    email_recipient = const.my_business_email
    email_subject = f'Failed to get Sandbox usage on realm aaay since {credits_start_date} in cron job - {script_name}'
    email_text = f'Failed to get Sandbox usage on realm aaay since {credits_start_date} in cron job - {script_name}. Check the attached logs'
    email_attachment = str(Path.cwd() / script_log_file)
    send_email(email_recipient, email_subject, email_text, email_attachment)
    raise

logger.debug(f'Fifth request endpoint answer is {sandbox_usage_on_realm_aaay_since2021_03_01}')

realm_aaay_minutes_up210301 = sandbox_usage_on_realm_aaay_since2021_03_01['data']['minutesUp']
realm_aaay_minutes_down210301 = sandbox_usage_on_realm_aaay_since2021_03_01['data']['minutesDown']
credits_spent_on_realm_aaay = realm_aaay_minutes_up210301 + (realm_aaay_minutes_down210301 * 0.3)
log_and_print(credits_spent_on_realm_aaay)
...
last_dataframe_row = {'SandboxID': '', 'Realm': '', 'Instance': '', 'State': '', 'Created at': '', 'Created by': '',
                      ...
                      'Credits since 01.03.2021': credits_spent_on_realm_aaay}
...
email_subject = 'ASDA Web ODS update for today'
