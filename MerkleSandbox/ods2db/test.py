...
# Set script name
script_name = 'clarins-ods-usage-2db'
...
for i in sandbox_list['data']:
    ...
    if realm == 'aafs':  # Smenyash ID-to mezhdu kavichkite s tova na klienta, koito iskash da reportvash. ID-to se vzima ot https://wiki.isobarsystems.com/display/SUPTEAM/Local+POD+time+-+Notifications
        ...
        try:
            ...
            email_recipient = const.my_business_email
            email_subject = f'Failed to get Sandbox usage on realm aafs since {credits_start_date} in cron job - {script_name}'
            email_text = f'Failed to get Sandbox usage on realm aafs since {credits_start_date} in cron job - {script_name}. Check the attached logs'
            email_attachment = str(Path.cwd() / script_log_file)
            send_email(email_recipient, email_subject, email_text, email_attachment)
            raise

        log_and_print(f'Fifth request endpoint answer is {sandbox_usage_on_realm_aafs_since2021_01_01}')
        ...
        list2fill.append(credits_spent_on_realm_aafs)
