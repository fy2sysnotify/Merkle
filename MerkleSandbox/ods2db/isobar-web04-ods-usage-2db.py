...
# Set script name
script_name = 'isobar-web04-ods-usage-2db'

# Set credits start date - must be in ISO 8601 format
credits_start_date = '2021-03-01'
...
for i in sandbox_list['data']:
    ...
    if realm == 'aakr':  # Smenyash ID-to mezhdu kavichkite s tova na klienta, koito iskash da reportvash. ID-to se vzima ot https://wiki.isobarsystems.com/display/SUPTEAM/Local+POD+time+-+Notifications
        realm_name = 'web04'  # Smenyash imeto mezhdu kavichkite s tova na klienta, koito iskash da reportvash.
        sandbox_id = i['id']
        ...
        createDate = created_at[:10]

        sandbox_usage_since_created = None

        try:
            sandbox_usage_since_created = requests.get(
                f"https://admin.us01.dx.commercecloud.salesforce.com/api/v1/sandboxes/{sandbox_id}/usage?from={createDate}",
        ...
        minutesDownSinceCreated = sandbox_usage_since_created['data']['minutesDown']
        list2fill.append(minutesDownSinceCreated)
        totalCredits = minutes_up_since_created + (minutesDownSinceCreated * 0.3)
        ...
        this_sandbox_usage_since2021_03_01 = None

        try:
            this_sandbox_usage_since2021_03_01 = requests.get(
        ...
        log_and_print(f'Fourth request endpoint answer is {this_sandbox_usage_since2021_03_01}')

        this_sandbox_minutes_up210301 = this_sandbox_usage_since2021_03_01['data']['minutesUp']
        this_sandbox_minutes_down210301 = this_sandbox_usage_since2021_03_01['data']['minutesDown']
        credits_spent_on_this_sandbox210301 = this_sandbox_minutes_up210301 + (this_sandbox_minutes_down210301 * 0.3)
        list2fill.append(credits_spent_on_this_sandbox210301)
        ...
        log_and_print(f'Fifth request endpoint - get Sandbox usage on realm aakr since {credits_start_date}')

        sandbox_usage_on_realm_aakr_since2021_03_01 = None

        try:
            sandbox_usage_on_realm_aakr_since2021_03_01 = requests.get(
                f"https://admin.us01.dx.commercecloud.salesforce.com/api/v1/realms/aakr/usage?from={credits_start_date}",
                headers=headers).json()
        except Exception as conn_error:
            logger.error(conn_error)
            log_and_print('Failed to get Sandbox usage on realm aakr from fifth request endpoint')
            email_recipient = const.my_business_email
            email_subject = f'Failed to get Sandbox usage on realm aakr since {credits_start_date} in cron job - {script_name}'
            email_text = f'Failed to get Sandbox usage on realm aakr since {credits_start_date} in cron job - {script_name}. Check the attached logs'
            email_attachment = str(Path.cwd() / script_log_file)
            send_email(email_recipient, email_subject, email_text, email_attachment)
            raise

        log_and_print(f'Fifth request endpoint answer is {sandbox_usage_on_realm_aakr_since2021_03_01}')

        realm_aakr_minutes_up210301 = sandbox_usage_on_realm_aakr_since2021_03_01['data']['minutesUp']
        realm_aakr_minutes_down210301 = sandbox_usage_on_realm_aakr_since2021_03_01['data']['minutesDown']
        credits_spent_on_realm_aakr = realm_aakr_minutes_up210301 + (realm_aakr_minutes_down210301 * 0.3)
        list2fill.append(credits_spent_on_realm_aakr)
...
try:
    ...
    sql = 'INSERT INTO IsobarWeb04ODSUsage(TimeStamp, SandboxID, Realm, Instance, State, CreatedAt, CreatedBy, MinutesUpSinceCreated, MinutesDownSinceCreated, TotalCredits, MinutesUpInLast24Hours, MinutesDownInLast24Hours, ThisODSCreditsSince01032021, TotalCreditsInRealmSince01032021) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'
