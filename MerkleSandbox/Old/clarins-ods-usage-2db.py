...
# Set script name
scriptName = 'clarins-ods-usage-2db'
...
for i in sandboxList['data']:
    ...
    if realm == 'aafs':  # Smenyash ID-to mezhdu kavichkite s tova na klienta, koito iskash da reportvash. ID-to se vzima ot https://wiki.isobarsystems.com/display/SUPTEAM/Local+POD+time+-+Notifications
        realmName = 'Clarins'  # Smenyash imeto mezhdu kavichkite s tova na klienta, koito iskash da reportvash.
        sandboxId = i['id']
        ...
        try:
            thisSandboxUsageSince2021_01_01 = requests.get(
        ...
        log_and_print(f'Fourth request endpoint answer is {thisSandboxUsageSince2021_01_01}')

        thisSandboxMinutesUp210101 = thisSandboxUsageSince2021_01_01['data']['minutesUp']
        thisSandboxMinutesDown210101 = thisSandboxUsageSince2021_01_01['data']['minutesDown']
        creditsSpentOnThisSandbox210101 = thisSandboxMinutesUp210101 + (thisSandboxMinutesDown210101 * 0.3)
        list2fill.append(creditsSpentOnThisSandbox210101)
        ...
        log_and_print('Fifth request endpoint - get Sandbox usage on realm aafs since 2021-01-01')

        try:
            sandboxUsageOnRealmAafsSince2021_01_01 = requests.get(
                f"https://admin.us01.dx.commercecloud.salesforce.com/api/v1/realms/aafs/usage?from=2021-01-01",
                headers=headers).json()
        except Exception as conn_error:
            logger.error(conn_error)
            log_and_print('Failed to get Sandbox usage on realm aafs from fifth request endpoint')
            email_recipient = 'konstantin.yanev@isobar.com'
            email_subject = f'Failed to get Sandbox usage on realm aafs since 21.01.01 in cron job - {scriptName}'
            email_text = f'Failed to get Sandbox usage on realm aafs since 21.01.01 in cron job - {scriptName}. Check the attached logs'
            email_attachment = str(Path.cwd() / scriptLogFile)
            send_email(email_recipient, email_subject, email_text, email_attachment)

        log_and_print(f'Fifth request endpoint answer is {sandboxUsageOnRealmAafsSince2021_01_01}')

        realmAafsMinutesUp210101 = sandboxUsageOnRealmAafsSince2021_01_01['data']['minutesUp']
        realmAafsMinutesDown210101 = sandboxUsageOnRealmAafsSince2021_01_01['data']['minutesDown']
        creditsSpentOnRealmAafs = realmAafsMinutesUp210101 + (realmAafsMinutesDown210101 * 0.3)
        list2fill.append(creditsSpentOnRealmAafs)
...
try:
    ...
    sql = ('INSERT INTO ClarinsODSUsage(TimeStamp, SandboxID, Realm, Instance, State, CreatedAt, CreatedBy, MinutesUpSinceCreated, MinutesDownSinceCreated, TotalCredits, MinutesUpInLast24Hours, MinutesDownInLast24Hours, ThisODSCreditsSince01012021, TotalCreditsInRealmSince01012021) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)')
