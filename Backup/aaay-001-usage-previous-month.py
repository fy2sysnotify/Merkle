...
# Set script name
scriptName = 'aaay-001-usage-previous-month'

# sandbox Id to be used for reporting
sandboxId = '82a4552f-7e6a-4241-8c4a-275d148e0141'
...
try:
    last_month_usage = requests.get(
        f"https://admin.us01.dx.commercecloud.salesforce.com/api/v1/sandboxes/{sandboxId}/usage?from={usage_start_date}",
        headers=headers).json()
except Exception as conn_error:
    logger.error(conn_error)
    logger.debug('Failed to get Sandbox usage for the last month from api')
    email_recipient = 'konstantin.yanev@isobar.com'
    email_subject = f'Failed to get Sandbox usage in cron job - {scriptName}'
    email_text = f'Failed to get Sandbox usage in cron job - {scriptName}. Check the attached logs'
    email_attachment = str(Path.cwd() / scriptLogFile)
    send_email(email_recipient, email_subject, email_text, email_attachment)
...
lastDataframeRow = {'SandboxID': '', 'Minutes up for previous month': '', 'Minutes down for previous month': '', 'Credits for previous month': ''}
...
emailText = f'Hi Team. My name is {scriptName}. Please check Sandbox ID {sandboxId} usage for the previous month. Keep in mind that living sandboxes on demand are spending credits. Each minute uptime = 1 credit. And each minute down time = 0.3 credit. If you have any questions, please do not hesitate to contact Isobar Operations Support.'

email_recipient = 'kosyoyanev@gmail.com'
email_subject = f'SandboxId {sandboxId} usage for previous month'
