...
# Set script name
scriptName = 'merge-staging-properties'
...
# Set resources URL, local and remote folder name and create local folder and backup folder
log_and_print('Set resources URL, local and remote folder name and create local folder and backup folder')
sourceUrl = 'https://aaay-003.sandbox.us01.dx.commercecloud.salesforce.com/on/demandware.servlet/webdav/Sites/Dynamic/zwilling-ca'
...
webdav.verify = False
log_and_print('Start downloading resources folder from WebDav')
print(time.asctime(time.localtime(time.time())))
webdavDownloadStart = time.perf_counter()

try:
    webdav.download_directory(folderName, folderName)
except :
    log_and_print(f'Webdav download in {scriptName} failed')
    email_recipient = 'konstantin.yanev@isobar.com'
    email_subject = f'Webdav download in {scriptName} failed'
    email_text = f'Webdav download in {scriptName} failed'
    email_attachment = str(Path.cwd() / scriptLogFile)
    send_email(email_recipient, email_subject, email_text, email_attachment)
    raise

webdavDownloadEnd = time.perf_counter()
log_and_print(f'Finished downloading resources folder from WebDav in {sourceUrl}')
print(time.asctime(time.localtime(time.time())))
log_and_print(f'Resource folder downloaded for {round(webdavDownloadEnd - webdavDownloadStart, 2)} seconds.')

outputFolder = 'OutputFolder'
os.mkdir(outputFolder)
backupFolder = f'BackupFolder_{current_time}'
os.mkdir(backupFolder)
subfolder2Remove = '.history'
devFolder = 'Release'

# Remove .history subfolder if it exists
log_and_print('Remove subfolder .history if exists')
dirPath = Path(folderName, subfolder2Remove)
if dirPath.exists() and dirPath.is_dir():
    shutil.rmtree(dirPath)

# Copy content of downloaded folder to backup folder
log_and_print('Copy content of downloaded folder to backup folder')
shutil.copytree(folderName, backupFolder, dirs_exist_ok=True)
log_and_print('Zip backup folder and delete it')
zip_archive = f'backupFolder_{current_time}'
shutil.make_archive(backupFolder, 'zip', zip_archive)
...
log_and_print('Concatenate the content of the files in each directory and write the merged content into a file with the same name at the current working directory')
...
for propertyFile in glob.iglob('*.properties'):
    shutil.move(propertyFile, outputFolder)

# Remove 'resources' local folder
log_and_print(f'Remove {folderName} local folder')
shutil.rmtree(folderName)

# Send email for job success with log attached
email_recipient = 'konstantin.yanev@isobar.com'
email_subject = f'{scriptName} job has been completed successfully'
email_text = f'{scriptName} job has been completed successfully'
email_attachment = str(Path.cwd() / scriptLogFile)
send_email(email_recipient, email_subject, email_text, email_attachment)