...
logging.basicConfig(filename=f'{scriptName}_{todayIs}.log',
                    level=logging.DEBUG,
                    format=logFormat)
...
try:
    ...
    email_recipient = 'konstantin.yanev@isobar.com'
    email_subject = f'Failed to get Sandbox usage on realm aafs since 21.01.01 in cron job - {scriptName}_sfcc'
    email_text = f'Failed to get Sandbox usage on realm aafs since 21.01.01 in cron job - {scriptName}_sfcc. Check the attached logs'
    email_attachment = str(Path.cwd() / scriptLogFile)
    send_email(email_recipient, email_subject, email_text, email_attachment)

logger.debug(f'Fifth request endpoint answer is {sandboxUsageOnRealmAafsSince2021_01_01}')
