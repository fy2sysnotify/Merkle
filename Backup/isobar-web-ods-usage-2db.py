...
# Create and configure logger
logFormat = '%(levelname)s %(asctime)s - %(message)s'
logging.basicConfig(filename=f'isobar-web-ods-usage-2db-{todayIs}.log',
                    level=logging.DEBUG,
                    format=logFormat)
logger = logging.getLogger()
...
try:
    ...
    email_recipient = 'konstantin.yanev@isobar.com'
    email_subject = 'Failed to get authorization for access token in isobar-web-ods-usage-2db'
    email_text = 'Failed to get authorization for access token in isobar-web-ods-usage-2db. Check with VPN if virtual machine (https://asofivcenter01.srv.ecm/ui/) is up and running. Check if SFCC Authenticator is active. Check if fake GPS is up and running.'
    email_attachment = str(Path.cwd() / f'isobar-web-ods-usage-2db-{todayIs}.log')
    send_email(email_recipient, email_subject, email_text, email_attachment)
    raise
...
try:
    ...
    logger.debug('Some of the credentials are invalid. Check them carefully')
    email_recipient = 'konstantin.yanev@isobar.com'
    email_subject = 'Some of the credentials in cron job - isobar-web-ods-usage-2db are invalid'
    email_text = 'Failed to get access token in cron job - isobar-web-ods-usage-2db. Some of the credentials are invalid. Check them carefully.'
    email_attachment = str(Path.cwd() / f'isobar-web-ods-usage-2db{todayIs}.log')
    send_email(email_recipient, email_subject, email_text, email_attachment)
    raise
logger.debug('We got the access token')
...
try:
    ...
    email_recipient = 'konstantin.yanev@isobar.com'
    email_subject = 'Failed to get Sandbox lists in cron job - isobar-web-ods-usage-2db'
    email_text = 'Failed to get Sandbox lists in cron job - isobar-web-ods-usage-2db.'
    email_attachment = str(Path.cwd() / f'isobar-web-ods-usage-2db{todayIs}.log')
    send_email(email_recipient, email_subject, email_text, email_attachment)
    raise

logger.debug(f'First request endpoint answer is {sandboxList}')
print(f'First request endpoint answer is {sandboxList}')
...
for i in sandboxList['data']:
    ...
    if realm == 'aaay':  # Smenyash ID-to mezhdu kavichkite s tova na klienta, koito iskash da reportvash. ID-to se vzima ot https://wiki.isobarsystems.com/display/SUPTEAM/Local+POD+time+-+Notifications
        ...
        # Make request with sandboxIDs to get Sandbox usage since inception
        print('Second request endpoint - get Sandbox usage since inception')
        logger.debug('Second request endpoint - get Sandbox usage since inception')
        ...
        try:
            ...
            email_recipient = 'konstantin.yanev@isobar.com'
            email_subject = 'Failed to get Sandbox usage since inception in cron job - isobar-web-ods-usage-2db'
            email_text = 'Failed to get Sandbox usage since inception in cron job - isobar-web-ods-usage-2db. Check the attached logs'
            email_attachment = str(Path.cwd() / f'isobar-web-ods-usage-2db-{todayIs}.log')
            send_email(email_recipient, email_subject, email_text, email_attachment)

        logger.debug(f'Second request endpoint answer is {sandboxUsageSinceCreated}')
        print(f'Second request endpoint answer is {sandboxUsageSinceCreated}')
        ...
        try:
            ...
            email_recipient = 'konstantin.yanev@isobar.com'
            email_subject = 'Failed to get Sandbox usage for the last 24 hours in cron job - isobar-web-ods-usage-2db'
            email_text = 'Failed to get Sandbox usage for the last 24 hours in cron job - isobar-web-ods-usage-2db. Check the attached logs'
            email_attachment = str(Path.cwd() / f'isobar-web-ods-usage-2db-{todayIs}.log')
            send_email(email_recipient, email_subject, email_text, email_attachment)

        logger.debug(f'Third request endpoint answer is {sandboxUsageLast24}')
        print(f'Third request endpoint answer is {sandboxUsageLast24}')
        ...
        try:
            ...
            logger.debug('Failed to get Sandbox usage from fourth request endpoint')
            email_recipient = 'konstantin.yanev@isobar.com'
            email_subject = 'Failed to get Sandbox usage since 21.03.01. in cron job - isobar-web-ods-usage-2db'
            email_text = 'Failed to get Sandbox usage since 21.01.03. in cron job - isobar-web-ods-usage-2db. Check the attached logs'
            email_attachment = str(Path.cwd() / f'isobar-web-ods-usage-2db-{todayIs}.log')
            send_email(email_recipient, email_subject, email_text, email_attachment)

        logger.debug(f'Fourth request endpoint answer is {thisSandboxUsageSince2021_03_01}')
        print(f'Fourth request endpoint answer is {thisSandboxUsageSince2021_03_01}')
        ...
        # Get usage for realm aaay since 01.03.2021
        print('Fifth request endpoint - get Sandbox usage on realm aaay since 2021-03-01')
        logger.debug('Fifth request endpoint - get Sandbox usage on realm aaay since 2021-03-01')
        ...
        try:
            ...
            email_recipient = 'konstantin.yanev@isobar.com'
            email_subject = 'Failed to get Sandbox usage on realm aaay since 21.03.01 in cron job - isobar-web-ods-usage-2db'
            email_text = 'Failed to get Sandbox usage on realm aaay since 21.03.01 in cron job - isobar-web-ods-usage-2db. Check the attached logs'
            email_attachment = str(Path.cwd() / f'isobar-web-ods-usage-2db-{todayIs}.log')
            send_email(email_recipient, email_subject, email_text, email_attachment)

        logger.debug(f'Fifth request endpoint answer is {sandboxUsageOnRealmAaaySince2021_03_01}')
        print(f'Fifth request endpoint answer is {sandboxUsageOnRealmAaaySince2021_03_01}')
...
try:
    ...
    db.rollback()
    logger.error(dbError)
    logger.debug('Failed to connect or insert to database in isobar-web-ods-usage-2db')
    email_recipient = 'konstantin.yanev@isobar.com'
    email_subject = 'Failed to connect or insert to database in isobar-web-ods-usage-2db'
    email_text = 'Failed to connect or insert to database in isobar-web-ods-usage-2db.'
    email_attachment = str(Path.cwd() / f'isobar-web-ods-usage-2db-{todayIs}.log')
    send_email(email_recipient, email_subject, email_text, email_attachment)
    raise
