...
def get_failed_orders(site_id: str) -> Dict[str, Any]:
    ...
    :param: site_id (str): The ID of the site to search for failed orders.
...
def main() -> None:
    ...
    The function iterates through the zwilling_instances and retrieves the failed orders for each site.
    It then counts the number of failed orders for each site and compares it to the corresponding
    value in the zwilling_instances dict. If the number of failed orders is greater than the corresponding
    value in zwilling_instances, it logs a message and sends a Slack notification with the details.
    :return: None
    """
    for site_id in zwilling_instances:
        ...
        if failed_orders_count > zwilling_instances[site_id]:
            country_id = site_id.removeprefix('zwilling-')
