...
def slack_post(slack_channel: str, slack_message: str):
    ...
    :param: slack_channel (str): Channel name to send the message to. It must start with #
    :param: slack_message (str): The message to post to the Slack channel.
    :return: None if the message was posted successfully, or an Exception object if an error occurred.
    """
    try:
        slack_client = slack_sdk.WebClient(token=config('SLACK_BOT_GRAFANA_API_KEY', default=''))
...
def upload_to_slack(slack_channel: str, file_to_upload: Union[str, bytes], slack_comment: str):
    ...
    :param: slack_channel (str): Channel name to send the message to. It must start with #
    :param: file_to_upload (Union[str, bytes]): The file to upload. Can be a file path string or a bytes object.
    :param: slack_comment (str): A comment to include with the uploaded file.
