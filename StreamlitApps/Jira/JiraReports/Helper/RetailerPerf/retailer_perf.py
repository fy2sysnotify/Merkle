...
from StreamlitApps.Jira.JiraReports.ConfigModules.projects_config import PROJECT_KEYS, PROJECT_NAMES
from StreamlitApps.Jira.JiraReports.ConfigModules.available_priorities import issue_priorities
    ...
    def count_issues_per_project(self, issues: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        issue_count = {
            project: {
                ...
                "customfield": {value: {"total": 0, "priority": {priority: 0 for priority in issue_priorities}} for
                                value in customfield_10200.values()}  # Count by custom fields
            }
        ...
        for issue in issues:  # Iterate over each issue
            ...
            if not isinstance(custom_field_values, list):
                custom_field_values = [custom_field_values]  # Ensure it's a list
                    ...
                    if isinstance(custom_field, dict):
                        ...
                        if custom_field_label not in issue_count[project_key]["customfield"]:
                            issue_count[project_key]["customfield"][custom_field_label] = {
                                "total": 0, "priority": {priority: 0 for priority in issue_priorities}}  # Initialize
...
class DataFrameTransformer:
    ...
    def create_issue_specific_dataframe(issues: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        :rtype: pd.DataFrame
        """
        logger.info("Creating dataframe for specific issues with customfield_10200 set")
