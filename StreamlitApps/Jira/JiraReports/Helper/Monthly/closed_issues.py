...
class IssueReporter:
    ...
    def print_report(self) -> None:
        """
        Print the generated report to the logger.

        :raises Exception: If an error occurs while printing the report.
        """
        try:
            for month, projects in self.issues_count_per_month.items():
                # Log the month
                logger.info(f'{month}:')
                for project_key, count in projects.items():
                    # Log the issue count for each project
                    logger.info(f'  {project_key}: {count}')
                # Log the total number of issues for the month
                logger.info(f'Total: {self.monthly_totals[month]}')

        except Exception as e:
            # Log an error message if printing the report fails
