...
from StreamlitApps.Jira.JiraReports.ConfigModules.projects_config import PROJECT_KEYS, \
    PROJECT_NAMES  # Custom module to get project keys and names
from StreamlitApps.Jira.JiraReports.ConfigModules.available_priorities import \
    issue_priorities  # Custom module to get available issue priorities
from StreamlitApps.Jira.JiraReports.ConfigModules.my_logger import configure_logger  # Custom logger configuration

logger = configure_logger(script_name=Path(__file__).stem)  # Configuring the logger
...
class JQLQueryBuilder:
    ...
    def _build_case_type_issue_queries(self,
        ...
        return {
            ...
            "not_closed_up_to_30": self._build_jql_query(
                projects_query,
                f'issuetype = "Support Request" AND "Case Type" = Issue AND '
                f'status != Closed AND created >= '
                f'"{self.date_ranges["not_closed_up_to_30"][0].strftime("%Y-%m-%d")}" '
                f'AND created <= '
                f'"{self.date_ranges["not_closed_up_to_30"][1].strftime("%Y-%m-%d")}"'
            ),
...
class JiraConnector:
    ...
    def connect(self) -> JIRA:
        ...
        """
        try:
            logger.info("Connecting to JIRA...")  # Log the connection attempt
            return JIRA(server=self.jira_server, token_auth=self.jira_api_token)  # Connect to JIRA
        except Exception as exc:
            logger.error(f"Failed to connect to JIRA: {exc}")  # Log the error if connection fails
...
class IssueFetcher:
    ...
    def fetch_issues_with_jql(self, jql_query: str) -> List[Issue]:
        ...
        while True:  # Loop to handle pagination
            try:
                logger.info(f"Fetching issues for JQL starting from {start_at}")  # Log the current start point
                ...
                logger.exception(f"Failed to fetch issues for JQL: {jql_query} "
                                 f"with exception: {ex}")  # Log the exception if fetching fails
                break  # Break the loop in case of an error

        logger.info(f"Total issues fetched: {len(all_issues_list)}")  # Log the total number of fetched issues
