...
def main() -> None:
    ...
    for sheet_name, jql in JQL_QUERIES.items():
        ...
        # Calculate the averages for resolution times
        # average_minutes = df["Resolved Time (Minutes)"].dropna().mean() if not df[
        #     "Resolved Time (Minutes)"].isnull().all() else None
        average_hours = df["Resolved Time (Hours)"].dropna().mean() if not df[
            "Resolved Time (Hours)"].isnull().all() else None
        # average_days = df["Resolved Time (Days)"].dropna().mean() if not df[
        #     "Resolved Time (Days)"].isnull().all() else None

        # Create a row for averages and append it to the DataFrame
        average_row = {
            "Key": "Average",
            "Summary": "",
            "Created": "",
            "Resolution Date": "",
            # "Resolved Time (Minutes)": average_minutes,
            "Resolved Time (Hours)": average_hours,
            # "Resolved Time (Days)": average_days
        }
    ...
    if excel_data:
        ...
        # Send email notification with the Excel report attachment
        try:
            send_email('kosyoyanev2@gmail.com, kosyoyanev3@gmail.com',
                       'ASDA FLS Jira Issues Created and Resolved in Previous month',
                       f'Hi Team, you may find the attached Excel report with ASDA FLS '
                       f'issues created and resolved in the previous month.',
                       excel_file,
        ...
        try:
            os.remove(excel_file)  # Correct method for deleting a file
            print(f"File {excel_file} deleted successfully.")
        except Exception as e:
            print(f"Error deleting file {excel_file}: {e}")
